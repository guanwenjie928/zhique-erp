...
const STATUS_OPTIONS = [
  ...
  { value: 'partial_receipt', label: '部分入库' },
  { value: 'receipted', label: '已入库' },
...
export default function PurchaseOrderList() {
  ...
  const handleAudit = async (record) => {
    confirmAudit(record.bill_no, async () => {
      try {
        await purchaseOrderApi.audit(record.id)
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'buyer_name',
      width: 100,
      render: (text, record) => text || record.buyer?.real_name || '-',
    },
    {
      title: '状态',
      dataIndex: 'status',
        ...
        <Space size="small">
          ...
          {record.status === 'audited' && (
            <Button
              type="link"
              size="small"
              className="btn-push-down-receipt"
              icon={<DownOutlined />}
  ...
  return (
    <div className="page-purchase-order-list">
      <Card
        title={<Title level={4} style={{ margin: 0 }}>采购订单</Title>}
        extra={
          <Button
            type="primary"
            icon={<PlusOutlined />}
